...
const acquireLock = (scanner, scan) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const existing = yield lockRepo.safeFindById(scan.caller, lock.lock);
    if (existing != null) {
        return null;
    }
    else {
        const created = yield lockRepo.create(scan.caller, lock);
        const confirmed = yield lockRepo.safeFindById(scan.caller, lock.lock);
...
const releaseLock = (scan, lock) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield lockRepo.remove(scan.caller, lock);
});
const fileExt = (path) => {
    if (!path)
        return "";
...
//# sourceMappingURL=storageScan.js.map